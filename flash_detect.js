...
function detectFlash(requiredVersion) { 
  // var requiredVersion = Version the user needs to view site (max 6, min 2) 
...
function displayFlash(var_swf, var_ver, var_W, var_H , var_title, var_alt_img, var_alt_url ) {
	detectFlash(var_ver);  // call our detector now that it's safely loaded.
	// -->	// In this section we set up the content to be placed dynamically on the page.
	// Customize movie tags and alternate html content below.
	// ARGS:
	//		var_swf		= name of flash file
	//		var_ver		= plugin version of flash file
	//		var_W		= Width of Flash File
	//		var_H		= Height of Flash File
	//		var_title	= Title of HP Container for this Flash File
	//		var_alt_img	= Alternate Image to use if user does not have current Flash plugin installed
	//		var_alt_url	= Alternate URL to use if user does not have current Flash plugin installed
	
	if (!useRedirect) {    // if dynamic embedding is turned on
	  if(hasRightVersion && var_swf != "") {  // if we've detected an acceptable version
			var oeTags = '<OBJECT CLASSID="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000"'
			+ 'WIDTH="'+ var_W +'" HEIGHT="'+ var_H +'"'
			+ 'CODEBASE="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab">'
			+ '<PARAM NAME="MOVIE" VALUE="/uploads/flash/'+ var_swf+'">'
			+ '<PARAM NAME="QUALITY" VALUE="high">'
			+ '<EMBED SRC="/uploads/flash/'+ var_swf+'"'
			+ 'WIDTH="'+ var_W +'" HEIGHT="'+ var_H +'"'
			+ 'QUALITY="high"'
	    ...
	    var alternateContent = '<a href="'+ var_alt_url +'"><img src="'+ var_alt_img +'" border="0" alt="'+ var_title +'"></a>'
